<template>
  <div class="c-shopmenu-cart-bar">
      ...
      <div class="l_auto cart-info" @click.stop.prevent="showList">
        ...
        <div class="cart-price">
          ...
          <div v-if="deliver.isFee">
            <i :class="{'free': cartData.price >= deliver.freeDeliverFee}">配送费 <em>¥ {{deliver.originFee | formatCurrency}}</em>（满{{deliver.freeDeliverFee | formatCurrency | noZeroCurrency}}免配送费）</i>
          </div>
        </div>
      </div>
    ...
    <div class="cart-list" v-show="visibleList" transition="totop">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      goNextView () {
        this.$router.go({name: 'createOrder', params: {mchnt_id: this.$parent.mchnt_id}})
