<template>
  <header>
    ...
    <button :disabled="btnDisabled" @click="getLocation()" type="button">重新定位</button>
  </header>
</template>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created () {
      let _t = setTimeout(() => {
        if (window.localStorage.getItem('longitude')) {
          Wechat.getFormattedAddress().then(this.setFormattedAddress)
        } else {
          this.getLocation()
        }
        clearTimeout(_t)
    ...
    methods: {
      getLocation () {
        this.formattedAddress = '未获取到地理位置'
