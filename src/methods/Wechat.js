    ...
    .then((response) => {
      ...
      if (code) {
        ...
        url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${data.appid}&redirect_uri=` +
          ...
          '&response_type=code&scope=snsapi_userinfo&state=' + encodeURIComponent(window.location.origin + window.location.pathname) + `&component_appid=${data.component_appid}#wechat_redirect`
      }
